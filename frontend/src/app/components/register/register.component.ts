...
import { RegisterStart } from 'src/app/redux/auth/auth-actions-types';
import { AppState } from 'src/app/redux/auth/auth-types';
...
export class RegisterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.firstFormGroup = this._formBuilder.group({
      emailCtrl: ['', [Validators.required, Validators.email], [this.validateEmail(this.authService)]],
  ...
  validateEmail = (authService: AuthService) => (c: FormControl) => {

    return authService.checkIfEmailExists(c.value)
      .pipe(take(1),
        map((isTaken: boolean) => {
          return isTaken
            ? { EmailAlreadyExistsInDb: true } : null;
        }),
      );
  };
